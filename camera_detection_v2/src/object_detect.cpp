...
ObjectDetect::ObjectDetect(QJsonObject objJson)
{
    graphPath = "./graph/frozen_inference_graph.pb";
    labelsPath = "../demo/ssd_mobilenet_v1_egohands/labels_map.pbtxt";
    ...
    // init labelsMap
    labelsMap = std::map<int, std::string>();
    QJsonObject labels = objJson["labels"].toObject();
    int len = labels.size();
    for (int i = 1; i <= len; i++) {
        string tmp = labels[std::to_string(i).c_str()].toString().toStdString();
        cout << tmp << endl;

        labelsMap.insert(pair<int, string>(i, tmp));
    }
